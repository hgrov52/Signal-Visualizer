%% Usage: sonogw(filespec, offset, duration, nfft, winlen, wintype, wininc,
%%            [lowfreq, [highfreq, [weight, [comment, [options, [scale]]]]]])
%%
%% sonogw plots a sonogram of an audio signal. The following
...
%%          wav filespec: sonogw({'test.wav', 2}, ...);
%%
...
%%          raw filespec: sonogw({'test.raw', 44100, 'short'}, ...);
%%
...
%% Examples:
%%
%% sonogw('test.wav', 1.5, 0.5, 1024, 1024, 'hanning', 512, 20, 20000,
%%            1, 'This is a test.', 'logfreq,magdb');
%%
%% sonogw({'test.raw', 96000, 'float'}, 1.5, 0.5, 1024, 1024, 'hanning',
%%            512, 20, 20000, 10, 'This is another test.');
%%
%% sonogw({'test.wav', 2}, 0, 5, 1024, 1024, 'hanning',
%%            512, 20, 20000, 10, 'Sonogram of right channel', 'sonogram');

function sonogw(filespec='song.wav', toffset=0, dur=5, nfft=1024, winlen=512, wintype='hanning', wininc=512,
...
if (nargin >= 12)
    if (ischar(options) == 0)
        error('sonogw: illegal argument type');
    endif
...
if (nargin == 13)
    ...
    if (length(scale) != 2)
        error('sonogw: illegal scale vector size');
    endif
    lowscale = scale(1);
    highscale = scale(2);
endif
...
%    axis([x, y]);
    ...
    [xx, yy] = meshgrid(x, y);
    surf(xx, yy, z*(1/max(max(z))), 'EdgeColor', 'none');
    view(0, 90);
%    colormap(flipud(gray));
endif;
...
elseif (o_notitle == 1)
    ...
    title(sprintf(strcat('\nSonogw plot %s, file: %s, sampling rate: %g Hz,\n',
...
if (o_logfreq == 1)
    if (lowfreq <= 0)
        warning('sonogw: lowfreq must be > 0 for logarithmic frequency plot');
